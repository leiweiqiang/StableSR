...
def create_test_subset(gt_dir, val_dir, num_samples=3):
    """创建测试子集"""
...
def test_tra_report_basic():
    ...
    try:
        ...
        if matching_files:
            ...
            # 加载图片
            val_tensor = tra_report._load_img(val_path)
            gt_tensor = tra_report._load_img(gt_path)
            
            print(f"输入图片尺寸: {val_tensor.shape}")
            print(f"目标图片尺寸: {gt_tensor.shape}")
...
def test_enhanced_tra_report_edge_only():
    ...
    try:
        ...
        enhanced_tra_report.load_models()
        ...
        if matching_files:
            ...
            # 加载图片
            val_tensor = enhanced_tra_report._load_img(val_path)
            gt_tensor = enhanced_tra_report._load_img(gt_path)
            ...
            upscaled_edge = enhanced_tra_report._upscale_with_edge_model(val_tensor)
            print(f"Edge上采样结果尺寸: {upscaled_edge.shape}")
            
            # 计算PSNR
            edge_psnr = enhanced_tra_report._calculate_psnr(upscaled_edge, gt_tensor)
