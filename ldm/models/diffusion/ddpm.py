...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
        ...
        # Initialize EdgeMapGenerator for Canny edge generation from GT images
        self.edge_generator = EdgeMapGenerator(device='cuda')
        print('EdgeMapGenerator initialized for Canny edge generation from GT images')
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # self.gt is already in [-1, 1] range after normalization
        canny_edge = self.edge_generator.generate_from_tensor(
            self.gt,                    # Input: GT image [B, 3, H, W] in [-1, 1]
                ...
                if len(input_list) == batch_size or col == grid_cols-1:
                    ...
                    if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                        if edge_map is not None:
                            ...
                            # Create a zero tensor as placeholder for edge_map with correct dimensions
                            # REMOVED: edge_tiles_cat creation - no longer needed
                            pass
                        ...
                        input_list_ = torch.cat([input_list] * 2)
                        t_in_ = torch.cat([t_in[:input_list.size(0)]] * 2)
                        # REMOVED: edge_map processing - no longer needed
        ...
        for i in iterator:
            ...
            if t_replace is not None:
                ...
                if edge_map is not None:
                    ...
                    # REMOVED: edge_input creation - no longer needed
                    pass
